...
class CallbackModule(CallbackBase):
    ...
    def v2_playbook_on_play_start(self, play):
        ...
        # self.print(str(dir(self.vm.get_vars())))
        # self.print(str(self.vm.get_vars()))
        # self.print(str(self.vm.get_vars().keys()))
        # self.print("================")
        # for x, y in self.vm.get_vars().items():
        #     self.print(f"{x} => {y}")
    ...
    def v2_playbook_on_stats(self, stats):
        ...
        if "xxx" in self.extra_vars:
            self.print(f"xxx extra_vars {self.extra_vars['xxx']}")
        elif "xxx" in self.play_vars:
            self.print(f"xxx play_vars {self.play_vars['xxx']}")
        else:
            self.print("xxx no")

        if "yyy" in self.extra_vars:
            self.print(f"yyy extra_vars {self.extra_vars['yyy']}")
        elif "yyy" in self.play_vars:
            self.print(f"yyy play_vars {self.play_vars['yyy']}")
        else:
            self.print("yyy no")

        if "zzz" in self.extra_vars:
            self.print(f"zzz extra_vars {self.extra_vars['zzz']}")
        elif "zzz" in self.play_vars:
            self.print(f"zzz play_vars {self.play_vars['zzz']}")
        else:
            self.print("zzz no")

        if "www" in self.extra_vars:
            self.print(f"www extra_vars {self.extra_vars['www']}")
        elif "www" in self.play_vars:
            self.print(f"www play_vars {self.play_vars['www']}")
        else:
            self.print("www no")
